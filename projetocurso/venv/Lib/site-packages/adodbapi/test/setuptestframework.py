...
def maketemp():
    ...
    ## try:  ## if running simultanous tests, don't erase the other thread's work
...
def _cleanup_function(testfolder, mdb_name):
    ...
    except: pass # tests package not present
...
# make a new package directory for the tests copy of ado
def makeadopackage(testfolder):
    ...
    if os.path.exists(adoName):
        ...
        if sys.version_info >= (3,0): # only when running Py3.n
            save = sys.stdout
            sys.stdout = None
            from lib2to3.main import main  # use 2to3 to make tests package
            main("lib2to3.fixes",args=['-n','-w', newpackage])
            sys.stdout = save
        return testfolder
    else:
        raise EnvironmentError('Connot find source of adodbapi to tests.')

def makemdb(testfolder, mdb_name):
    # following setup code borrowed from pywin32 odbc tests suite
    ...
    if not os.path.isfile(_accessdatasource):
        ...
        if dbe:
            ...
            print(('    ...copying tests ACCESS db to '+_accessdatasource))
            mdbName = os.path.normpath(os.getcwd() + '/../examples/tests.mdb')
            import shutil
