...
class CommonDBTests(unittest.TestCase):
    ...
    def testUserDefinedConversionForExactNumericTypes(self):
        ...
        #
        # !!! this tests intentionally alters the value of what should be constant in the module
        # !!! no new code should use this example, to is only a tests to see that the
        # !!! deprecated way of doing this still works.  (use connection.variantConversions)
        #
        if not self.remote and sys.version_info < (3,0): ### Py3 need different tests
    ...
    def helpCreateAndPopulateTableTemp(self,crsr):
        ...
        try:  #EAFP
            ...
            crsr.execute(tabdef)
        for i in range(9): # note: this poor SQL code, but a valid tests
            crsr.execute("INSERT INTO xx_%s (fldData) VALUES (%i)" % (config.tmp, i))
            # NOTE: building the tests table without using parameter substitution
    ...
    def testRowIterator(self):
        ...
        for inParam in inputs:
            ...
            for j in range(len(inParam)):
                assert rec[j] == inParam[j], 'returned value:"%s" != tests value:"%s"'%(rec[j],inParam[j])
            # check that we can get a complete tuple from a row
            assert tuple(rec) == inParam, 'returned value:"%s" != tests value:"%s"'%(repr(rec),repr(inParam))
            # tests that slices of rows work
            slice1 = tuple(rec[:-1])
            slice2 = tuple(inParam[0:2])
            assert slice1 == slice2, 'returned value:"%s" != tests value:"%s"'%(repr(slice1),repr(slice2))
            # now tests named column retrieval
            assert rec['fldTwo'] == inParam[0]
            assert rec.fldThree == inParam[1]
            assert rec.fldFour == inParam[2]
        # tests array operation
    ...
    def testFormatParamstyle(self):
        self.helpForceDropOnTblTemp()
        conn=self.getConnection()
        conn.paramstyle = 'format'  #tests nonstandard use of paramstyle
        crsr=conn.cursor()
        tabdef= """
            CREATE TABLE xx_%s (
                fldId integer NOT NULL,
        ...
        for inParam in inputs:
            ...
            rec = crsr.fetchone()
            self.assertEqual(rec[0], inParam, 'returned value:"%s" != tests value:"%s"' % (rec[0],inParam))
        ...
        #tests the .command attribute
        assert crsr.command == sel
        #tests the .parameters attribute
    ...
    def testNamedParamstyle(self):
        ...
        conn=self.getConnection()
        crsr=conn.cursor()
        crsr.paramstyle = 'named'  #tests nonstandard use of paramstyle
        tabdef= """
            CREATE TABLE xx_%s (
                fldId integer NOT NULL,
                fldData varchar(10))
        ...
        for inParam in inputs:
            ...
            rec = crsr.fetchone()
            self.assertEqual(rec[0], inParam, 'returned value:"%s" != tests value:"%s"'%(rec[0],inParam))
        # now a tests with a ":" as part of a literal
    ...
    def testPyformatParamstyle(self):
        ...
        conn=self.getConnection()
        crsr=conn.cursor()
        crsr.paramstyle = 'pyformat'  #tests nonstandard use of paramstyle
        tabdef= """
            CREATE TABLE xx_%s (
                fldId integer NOT NULL,
                fldData varchar(10))
    ...
    def testAutomaticParamstyle(self):
        self.helpForceDropOnTblTemp()
        conn=self.getConnection()
        conn.paramstyle = 'dynamic'  #tests nonstandard use of paramstyle
        crsr=conn.cursor()
        tabdef= """
            CREATE TABLE xx_%s (
                fldId integer NOT NULL,
        ...
        for inParam in inputs:
            ...
            rec = crsr.fetchone()
            self.assertEqual(rec[0], inParam, 'returned value:"%s" != tests value:"%s"'%(rec[0],inParam))
        # now a tests with a ":" as part of a literal -- and use a prepared query
    ...
    def testAutoRollback(self):
        ...
        try:
            ...
            row = None  # if the entire table disappeared the rollback was perfect and the tests passed
...
suites=[]
suites.append( unittest.makeSuite(TestPythonDateTimeConverter,'tests'))
if config.doMxDateTimeTest:
    suites.append( unittest.makeSuite(TestMXDateTimeConverter,'tests'))
if config.doTimeTest:
    suites.append( unittest.makeSuite(TestPythonTimeConverter,'tests'))

if config.doAccessTest:
    suites.append( unittest.makeSuite(TestADOwithAccessDB,'tests'))
if config.doSqlServerTest:    
    suites.append( unittest.makeSuite(TestADOwithSQLServer,'tests'))
if config.doMySqlTest:
    suites.append( unittest.makeSuite(TestADOwithMySql,'tests'))
if config.doPostgresTest:
    suites.append( unittest.makeSuite(TestADOwithPostgres,'tests'))
