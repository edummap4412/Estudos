...
class TestSimpleOps(unittest.TestCase):
    ...
    # A simple tests using normal read/write operations.
...
class TestOverlapped(unittest.TestCase):
    ...
    def testCompletionPortsNonQueued(self, test_overlapped_death = 0):
        ...
        # PostQueuedCompletionStatus.  This tests is based on the reproduction
        ...
        t.setDaemon(True) # avoid hanging entire tests suite on failure.
        t.start()
        try:
    ...
    def testComparable(self):
        ...
        # ensure we explicitly tests the operators.
...
class TestDirectoryChanges(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i in range(self.num_test_dirs):
            td = tempfile.mktemp("-tests-directory-changes-%d" % i)
            os.mkdir(td)
...
class TestConnect(unittest.TestCase):
    def connect_thread_runner(self, expect_payload, giveup_event):
        ...
        # accept, as our tests connection may never come.  May as well use
        # AcceptEx for this...
        ...
        # wait for the connection or our tests to fail.
        events = giveup_event, overlapped.hEvent
        rc = win32event.WaitForMultipleObjects(events, False, 2000)
        if rc == win32event.WAIT_TIMEOUT:
            self.fail("timed out waiting for a connection")
        if rc == win32event.WAIT_OBJECT_0:
            # Our main thread running the tests failed and will never connect.
            return
...
class TestWSAEnumNetworkEvents(unittest.TestCase):
    ...
    def test_functional(self):
        # This is not really a unit tests, but it does exercise the code
