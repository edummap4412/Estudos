...
try:
    ...
    k, disp=win32api.RegCreateKeyEx(win32con.HKEY_CURRENT_USER, 'Python tests key', SecurityAttributes=sa,
        samDesired=win32con.KEY_ALL_ACCESS, Class='some class', Options=0)
    win32api.RegSetValue(k, None, win32con.REG_SZ, 'Default value for python tests key')

    subk, disp=win32api.RegCreateKeyEx(k, 'python tests subkey', SecurityAttributes=sa,
    ...
    restored_key, disp=win32api.RegCreateKeyEx(win32con.HKEY_CURRENT_USER, 'Python tests key(restored)', SecurityAttributes=sa,
