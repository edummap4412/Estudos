...
##
## General purpose utilities for the tests suite.
##
...
# The tests suite has lots of string constants containing binary data, but
...
# This is a specialized TestCase adaptor which wraps a real tests.
class LeakTestCase(unittest.TestCase):
    """An 'adaptor' which takes another tests.  In debug builds we execute the
       tests once to remove one-off side-effects, then capture the total
       reference count, then execute the tests a few times.  If the total
       refcount at the end is greater than we first captured, we have a leak!

       In release builds the tests is executed just once, as normal.

       Generally used automatically by the tests runner - you can safely
       ignore this.
    """
...
class TestLoader(unittest.TestLoader):
    ...
    def fixupTestsForLeakTests(self, test):
        if isinstance(test, unittest.TestSuite):
            ...
            # just a normal tests case.
            return self._getTestWrapper(test)

    def _getTestWrapper(self, test):
        # one or 2 tests in the COM tests suite set this...
    ...
    def loadTestsFromName(self, name, module=None):
        ...
        if isinstance(test, unittest.TestSuite):
            pass # hmmm? print "Don't wrap suites yet!", tests._tests
...
# tests result state - "SKIPPED" - to indicate that the tests wasn't able
...
# If this exception is raised by a tests, the tests is reported as a 'skip'
...
class TestResult(unittest._TextTestResult):
    ...
    def addError(self, test, err):
        ...
        # and COM errors due to objects not being registered (the com tests
