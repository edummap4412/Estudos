...
# tests methods that are acceptable.
...
class Extension(threaded_extension.ThreadPoolExtension):
    ...
    def Dispatch(self, ecb):
        ...
        if meth is None:
            raise AttributeError("No tests named '%s'" % (test_name,))
        result = meth(ecb)
        if result is None:
            # This means the tests finalized everything
            return
        ecb.SendResponseHeaders("200 OK", "Content-type: text/html\r\n\r\n", 
                                False)
        print("<HTML><BODY>Finished running tests <i>", test_name, "</i>", file=ecb)
        print("<pre>", file=ecb)
    ...
    def test_long_vars(self, ecb):
        ...
        # Our implementation has a default buffer size of 8k - so we tests
        ...
        if qs == expected_query:
            return "Total length of variable is %d - tests worked!" % (len(qs),)
        else:
