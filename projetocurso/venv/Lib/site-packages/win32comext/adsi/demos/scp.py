...
Executing with --tests will create and remove one of everything.
"""
...
def main():
    ...
    parser.add_option("-t", "--tests", action="store_true",
                      help="Execute a mini-tests suite, providing defaults for most options and args"),
    ...
    if options.test:
        if args:
            parser.error("Can't specify args with --tests")
    
        args = "ScpDelete ScpCreate SpnCreate SpnRegister SpnUnregister ScpDelete"
        log(1, "--tests - pretending args are:\n %s", args)
        args = args.split()
        if not options.service_class:
            options.service_class = "PythonScpTest"
            log(2, "--tests: --service-class=%s", options.service_class)
        if not options.keywords:
            options.keywords = "Python Powered".split()
            log(2, "--tests: --keyword=%s", options.keywords)
        if not options.binding_string:
            options.binding_string = "tests binding string"
            log(2, "--tests: --binding-string=%s", options.binding_string)
