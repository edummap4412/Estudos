...
sub testcollection
   if tests.collection.Item(0) <> 1 then
     tests.fail("Index 0 was wrong")
   end if
   if tests.collection.Item(1) <> "Two" then
     tests.fail("Index 1 was wrong")
   end if
   if tests.collection.Item(2) <> 3 then
     tests.fail("Index 2 was wrong")
   end if
   num = 0
   for each item in tests.collection
     num = num + 1
   next
   if num <> 3 then
     tests.fail("Collection didn't have 3 items")
   end if
end sub
"""
PyScript = """\
...
def testcollection():
#   tests.collection[1] = "New one"
   got = []
   for item in tests.collection:
     got.append(item)
   if got != [1, "Two", 3]:
     tests.fail("Didn't get the collection")
   pass
"""
