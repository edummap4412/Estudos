...
def patch(
    ...
    decorating each tests method in the class. This reduces the boilerplate
    code when your tests methods share a common patchings set. `patch` finds
    tests by looking for method names that start with `patch.TEST_PREFIX`.
    By default this is `tests`, which matches the way `unittest` finds tests.
...
class _patch_dict(object):
    ...
    to its original state after the tests.
...
class AsyncMock(AsyncMockMixin, AsyncMagicMixin, Mock):
    ...
    The tests author can also specify a wrapped object with ``wraps``. In this
