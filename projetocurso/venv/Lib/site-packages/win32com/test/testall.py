...
class PyCOMTest(TestCase):
    no_leak_tests = True # done by the tests itself
    def testit(self):
...
# This is a list of "win32com.tests.???" module names, optionally with a
...
unittest_modules = [
        ...
        """testIterators testvbscript_regexp testStorage 
          ...
          testServers errorSemantics.tests testvb testArrays
          testClipboard testMarshal
        """.split(),
        # Level 2 tests.
        """testMSOffice.TestAll testMSOfficeEvents.tests testAccess.tests
           testExplorer.TestAll testExchange.tests
        """.split(),
        # Level 3 tests.
        """testmakepy.TestAll
        """.split()
...
unittest_other_modules = [
        # Level 1 tests.
        """win32com.directsound.tests.ds_test
        """.split(),
        # Level 2 tests.
        [],
        # Level 3 tests.
...
output_checked_programs = [
        # Level 1 tests.
        [
            ("cscript.exe /nologo //E:vbscript testInterp.vbs", "VBScript tests worked OK"),
...
def get_test_mod_and_func(test_name, import_failures):
    if test_name.find(".")>0:
        ...
        mod_name = test_name
        func_name = None
    fq_mod_name = "win32com.tests." + mod_name
    try:
...
# Return a tests suite all loaded with the tests we want to run
...
if __name__=='__main__':
    ...
    for opt, val in opts:
        if opt=='-v':
            verbosity += 1
    testLevel = 1 # default to quick tests
    test_names = []
    for arg in args:
        try:
            testLevel = int(arg)
    ...
    if verbosity:
        ...
        print("Executing level %d tests - %d tests cases will be run" \
    ...
    if import_failures:
        testResult.stream.writeln("*** The following tests modules could not be imported ***")
        ...
        testResult.stream.writeln("*** %d tests(s) could not be run ***" % len(import_failures))
    
    # re-print unit-tests error here so it is noticed
