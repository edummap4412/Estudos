...
# You can run this with no args, and a tests database will be generated.
...
def CreateTestAccessDatabase(dbname = None):
    # Creates a tests access database - returns the filename.
    if dbname is None:
    ...
    try:
        ...
        print("WARNING - Unable to delete old tests database - expect a COM exception RSN!")

    newdb = workspace.CreateDatabase( dbname, constants.dbLangGeneral, constants.dbEncrypt )

    # Create one tests table.
    ...
    tab1.MoveFirst()
    # We do a simple bookmark tests which tests our optimized VT_SAFEARRAY|VT_UI1 support.
    ...
    # But first check the tests is actually doing something!
    tab1.MoveLast()
    if tab1.Fields("First Name").Value != "Second":
        raise RuntimeError("Unexpected record is last - makes bookmark tests pointless!")
...
def DumpAccessInfo(dbname):
    ...
    if amod is None and dmod is None:
        ...
        sys.stderr.write("testAccess not doing dynamic tests, as generated code already exists\n")
...
def test(dbname = None):
    if dbname is None:
        ...
        print("A tests database at '%s' was created" % dbname)
