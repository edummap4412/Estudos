...
def TestEnum(quiet=None):
    ...
    if not quiet: print("Simple enum tests")
    o = MakeTestEnum()
    check = [1,'Two',3]
    TestEnumAgainst(o, check)

    if not quiet: print("sub-collection tests")
    sub = o[3]
    TestEnumAgainst(sub ,['Sub1', 2, 'Sub3'])

    # Remove the sublist for this tests!
    o.Remove(o.Count()-1)

    if not quiet: print("Remove item tests")
    del check[1]
    o.Remove(1)
    TestEnumAgainst(o, check)

    if not quiet: print("Add item tests")
    o.Add('New Item')
    check.append('New Item')
    TestEnumAgainst(o, check)

    if not quiet: print("Insert item tests")
    o.Insert(2, -1)
    check.insert(2, -1)
    TestEnumAgainst(o, check)

### This does not work!
#       if not quiet: print "Indexed replace item tests"
#       o[2] = 'Replaced Item'
...
#       TestEnumAgainst(o, check)
    ...
    if not quiet: print("Empty collection tests")
    o = MakeEmptyEnum()
    for item in o:
