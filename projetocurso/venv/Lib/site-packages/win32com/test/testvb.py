...
# to tests byref params for gateway IDispatch.
class TestObject:
...
def TestVB( vbtest, bUseGenerated ):
    ...
    if bUseGenerated:
        ...
        # Cant tests further types for this VariantPutref, as only
...
#                       raise error("Could not pass an omitted integer byref")
        ...
        # Can't tests IncrementVariantParam with the param omitted as it
...
def TestStructs(vbtest):
    ...
    # Finally, tests stand-alone structure arrays.
    ...
    # tests repr - it uses repr() of the sub-objects, so check it matches.
...
def TestObjectSemantics(ob):
    # a convenient place to tests some of our equality semantics
    assert ob==ob._oleobj_
    assert not ob!=ob._oleobj_
    # same tests again, but lhs and rhs reversed.
...
def TestAll():
    # Import the type library for the tests module.  Let the 'invalid clsid'
    # exception filter up, where the tests runner will treat it as 'skipped'
...
# Make this tests run under our tests suite to leak tests etc work
def suite():
    import unittest
