...
importMsg = "**** PyCOMTest is not installed ***\n  PyCOMTest is a Python tests specific COM client and server.\n  It is likely this server is not installed on this machine\n  To install the server, you must get the win32com sources\n  and build it using MS Visual C++"

error = Exception

# This tests uses a Python implemented COM server - ensure correctly registered.
...
class RandomEventHandler:
    ...
    def OnFireWithNamedParams(self, no, a_bool, out1, out2):
        # This tests exists mainly to help with an old bug, where named
...
def TestCommon(o, is_generated):
    ...
    # We want to explicitly tests > 32 bits.  py3k has no 'maxint' and
...
def TestGenerated():
    ...
    TestEvents(o, handler)
    progress("Finished generated .py tests.")
...
def _TestPyVariant(o, is_generated, val, checker = None):
    ...
    # Handle our safe-array tests - if the passed value is a list of variants,
...
def TestVTable(clsctx=pythoncom.CLSCTX_ALL):
    ...
    TestLocalVTable(ob)
    # Now tests it via vtable - use some C++ code to help here as Python can't do it directly yet.
