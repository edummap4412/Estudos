...
class InvalidBool(object):
    """To help tests invalid bool values."""
...
class DrawTestCase(unittest.TestCase):
    """Base class to tests draw module functions."""
...
class PythonDrawTestCase(unittest.TestCase):
    """Base class to tests draw_py module functions."""
...
class DrawEllipseMixin(object):
    ...
    def test_ellipse__thick_line(self):
        ...
        # cleanly defined. So only tests up to a few thicknesses before the
        # maximum thickness.
    ...
    def test_ellipse__bounding_rect(self):
        ...
        # Make a rect that is bigger than the surface to help tests drawing
            ...
            for attr in RECT_POSITION_ATTRIBUTES:
                ...
                for width, height in sizes:
                    ...
                    for thickness in (0, 1, 2, 3, min(width, height)):
                        surface.fill(surf_color)  # Clear for each tests.
...
# Commented out to avoid cluttering the tests output. Add back in if draw_py
...
class BaseLineMixin(object):
    ...
    def _rect_lines(rect):
        # Yields pairs of end points and their reverse (to tests symmetry).
...
class LineMixin(BaseLineMixin):
    """Mixin tests for drawing a single line.
    ...
    def test_line__bounding_rect(self):
        ...
        # and one smaller (to tests lines that span the surface).
        for size in ((width + 5, height + 5), (width - 5, height - 5)):
            surface = pygame.Surface(size, 0, 32)
            surf_rect = surface.get_rect()

            # Move the helper rect to different positions to tests line
...
class LinesMixin(BaseLineMixin):
    ...
    def test_lines__bounding_rect(self):
        ...
        for size in ((width + 5, height + 5), (width - 5, height - 5)):
            ...
            # Move pos_rect to different positions to tests line endpoints on
            # and off the surface.
            for pos in rect_corners_mids_and_center(surf_rect):
                pos_rect.center = pos
...
class AALineMixin(BaseLineMixin):
    """Mixin tests for drawing a single aaline.
...
class DrawAALineTest(AALineMixin, DrawTestCase):
    ...
    def _check_antialiasing(
        ...
        # it is important to tests also opposite direction, the algorithm
    ...
    def test_short_non_antialiased_lines(self):
        """tests very short not anti aliased lines in all directions."""
...
class AALinesMixin(BaseLineMixin):
    """Mixin tests for drawing aalines.
...
class DrawCircleMixin(object):
    ...
    def test_circle__bounding_rect(self):
        ...
        # when centering the tests circle on one of its corners, the circle is
        # drawn fully off the tests surface, but a rect bounding the circle
        # would still overlap with the tests surface.
