...
class JoystickModuleTest(unittest.TestCase):
    ...
    def test_init(self):
        """
        This unit tests is for joystick.init()
        It was written to help reduce maintenance costs
        and to help tests against changes to the code or
        different platforms.
        """
        pygame.quit()
        #tests that pygame.init automatically calls joystick.init
        pygame.init()
        self.assertEqual(pygame.joystick.get_init(), True)

        #tests that get_count doesn't work w/o joystick init
        #this is done before and after an init to tests
        #that init activates the joystick functions
        pygame.joystick.quit()
        with self.assertRaises(pygame.error):
            pygame.joystick.get_count()

        #tests explicit call(s) to joystick.init.
        #Also tests that get_count works once init is called
        iterations = 20
