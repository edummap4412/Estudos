"""Pygame unit tests suite package

Exports function run()

A quick way to run the tests suite package from the command line
is by importing the go submodule:

python -m "import pygame.tests" [<tests options>]

Command line option --help displays a usage message. Available options
correspond to the pygame.tests.run arguments.

The xxxx_test submodules of the tests package are unit tests suites for
individual parts of Pygame. Each can also be run as a main program. This is
useful if the tests, such as cdrom_test, is interactive.

For Pygame development the tests suite can be run from a Pygame distribution
root directory using run_tests.py. Alternately, tests/__main__.py can be run
directly.
